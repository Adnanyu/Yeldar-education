    ...
    return (
        <div className="space-y- container mx-auto mb-10 p-4 6xl:!container lg:max-w-[85vw] bg-gray-100">
            ...
            <div className="mt-3 flex items-center justify-center text-center">
                <h1 className="text-lg font-bold mb-6 md:text-2xl text-[#242D4B]">Best Turkish Private Universities</h1>
            </div>
            <div className="space-y-4"><div className="grid w-full grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
                {universities.map((uni, key) => (
                
            <a className="w-auto" href={`/universities/${uni.link}`} key={key}>
                    <div className="shadow-all shadow-all group rounded-2xl bg-white p-3 duration-300">
                        <div className="relative">
                            <div className=" overflow-hidden rounded-full border-4 bg-white p-2 md:size-28 ">
                                <img alt="alt" loading="lazy" width="500" height="500" decoding="async" data-nimg="1" className="size-full object-contain" src={uni.logo} style={{ color: 'transparent' }}/>
                            </div>
                        </div>
                        <div className="mt-8 p-4 text-start md:mt-16">
                            <div className="flex items-center justify-between">
                                    <h2 className="line-clamp-1 text-base font-bold text-[#242D4B] sm:text-xl md:text-2xl">{ uni.name }</h2>
                                </div>
                                <section className="space-y-3 pt-3">
                                    <div className="grid w-full grid-cols-2 gap-2 text-center text-xs text-gray-800 md:grid-cols-4 md:items-center md:text-sm">
                                        <div className="flex flex-col items-center rounded-lg bg-gray-400/10 p-1 md:space-y-4 md:p-2">
                                            ...
                                            <span className="text-[#242D4B] font-bold">{ uni.year_of_establishment }</span>
                                        </div>
                                        <div className="flex flex-col items-center rounded-lg bg-gray-400/10 p-1 md:space-y-4 md:p-2">
                                            <span className="text-wrap text-primary">World ranking</span>
                                            <span className="text-[#242D4B] font-bold">{ uni.ranking_globally }</span></div>
                                        <div className="flex flex-col items-center rounded-lg bg-gray-400/10 p-1 md:space-y-4 md:p-2">
                                            <span className="text-primary">Local ranking</span>
                                            <span className="text-[#242D4B] font-bold">{ uni.ranking_locally }</span>
                                        </div><div className="flex flex-col items-center rounded-lg bg-gray-400/10 p-1 md:space-y-4 md:p-2">
                                            <span className="text-primary">Number of majors</span><span className="text-[#242D4B] font-bold">{ uni.number_of_specializations }</span>
                                        </div>
                                    </div>
                                    <div className="flex w-full flex-col justify-between gap-4 rounded-lg bg-gray-400/10 p-1.5 text-xs text-gray-800 md:flex-row md:p-3 md:text-sm">
                                        <span className="text-[#242D4B] font-bold">Prices starting</span>
                                        <div className="flex justify-between gap-4">
                                            <div className="flex items-center gap-2">
                                                <span>From</span><span className="text-[#242D4B] font-bold">{ uni.prices_start_from }</span>
                                            </div>
                                            <div className="flex items-center gap-2">
                                                <span>To</span>
                                                <span className="text-[#242D4B] font-bold">{ uni.prices_to }</span>
                                            </div>
                                        </div>
                                    </div>
                                </section>
                                <div className="mt-2 flex justify-center">
                                    <button className="inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&amp;_svg]:pointer-events-none [&amp;_svg]:size-4 [&amp;_svg]:shrink-0 bg-primary text-primary-foreground hover:scale-x-105 duration-300 transition-all h-10 px-4 py-2 group-hover:bg-secondary">سجل الآن</button>
                                </div>
                            </div>
                        </div>
                </a>
